...
import { systemSettingsNavItems } from '@/lib/admin/systemSettingsNavigation';
import { grayColors } from '@/lib/theme';

/** Figma dual-column sidebar: 528px total, sub nav column is 264px. */
export const SYSTEM_SETTINGS_SUBNAV_WIDTH = 264;

const navIconSx = { fontSize: 24, color: '#667085' };

type SystemSettingsSubnavProps = {
  onClose: () => void;
  onMouseEnter?: () => void;
};

export default function SystemSettingsSubnav({
  onClose,
  onMouseEnter,
}: SystemSettingsSubnavProps) {
  const pathname = usePathname();

  return (
    <aside
      className="flex h-full shrink-0 flex-col border-r border-[#EAECF0] bg-white"
      style={{ width: SYSTEM_SETTINGS_SUBNAV_WIDTH }}
      onMouseEnter={onMouseEnter}
    >
      <div className="flex flex-col gap-4 px-4 pb-4 pt-8">
        <div className="flex justify-end">
          <IconButton
            aria-label="Close system settings menu"
            size="small"
            onClick={onClose}
            sx={{
              p: 1,
